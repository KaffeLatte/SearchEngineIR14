...
public class SearchGUI extends JFrame {
	...
	JRadioButtonMenuItem intersectionItem = new JRadioButtonMenuItem(
			"Intersection query");
	JRadioButtonMenuItem phraseItem = new JRadioButtonMenuItem("Phrase query");
	JRadioButtonMenuItem rankedItem = new JRadioButtonMenuItem(
			"Ranked retrieval");
	...
	JRadioButtonMenuItem combinationItem = new JRadioButtonMenuItem(
			"Combination");
	...
	private void createGUI() {
		...
		Action search = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String queryString = SimpleTokenizer.normalize(queryWindow
						.getText());
				...
				synchronized (indexLock) {
					results = indexer.index.search(query, queryType,
							rankingType, structureType);
				}
				...
				if (results != null) {
					...
					for (int i = 0; i < results.size(); i++) {
						buf.append(" " + i + ". ");
						String filename = indexer.index.docIDs.get(""
								+ results.get(i).docID);
						if (filename == null) {
		...
		Action relevanceFeedbackSearch = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ((results != null) && (queryType == Index.RANKED_QUERY)) {
					...
					boolean[] docIsRelevant = { false, false, false, false,
					...
					for (int i = 0; i < results.size(); i++) {
						...
						buf.append("   "
								+ String.format("%.5f", results.get(i).score)
								+ "\n");
					}
				} else {
	...
	private void index() {
		synchronized (indexLock) {
			...
			for (int i = 0; i < dirNames.size(); i++) {
				preProcessFiles(new File(dirNames.get(i)));

			}
			indexer.startProcessFiles(fileStrings);
			resultWindow.setText("\n  Done!");
		}
	};

	private void preProcessFiles(File f) {
		if (f.canRead()) {
			if (f.isDirectory()) {
				String[] fs = f.list();
				...
				if (fs != null) {
					for (int i = 0; i < fs.length; i++) {
						preProcessFiles(new File(f, fs[i]));
					}
				}
			} else {
				fileStrings.add(f.toString());
